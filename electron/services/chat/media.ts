 ...
 */
export async function getImageData(state: ChatServiceState, sessionId: string, msgId: string, createTime?: number): Promise<{ success: boolean; data?: string; isThumb?: boolean; liveVideoPath?: string; error?: string }> {
  try {
    ...
    if (result.localPath.startsWith('data:')) {
      ...
      return base64Data
        ? { success: true, data: base64Data, isThumb: result.isThumb, liveVideoPath: result.liveVideoPath }
